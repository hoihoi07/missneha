            ...
            if INDEX > COUNT:
                ...
                if download.status() not in [
                    ...
                    if download.status() == MirrorStatus.STATUS_DOWNLOADING:
                        msg += f"\n<b>📥 Dᴏᴡɴʟᴏᴀᴅᴇᴅ :</b> {get_readable_file_size(download.processed_bytes())}<b>\n💾 Size</b>: {download.size()}"
                    elif download.status() == MirrorStatus.STATUS_CLONING:
                        msg += f"\n<b>♻️ Cʟᴏɴɪɴɢ :</b> {get_readable_file_size(download.processed_bytes())}<b>\n<b>⚙️ Engine: ʀᴄʟᴏɴᴇ</b>\n💾 Size</b>: {download.size()}"
                    else:
                        msg += f"\n<b>📤 Uᴘʟᴏᴀᴅᴇᴅ :</b> {get_readable_file_size(download.processed_bytes())}<b>\n<b>⚙️ Engine: ʀᴄʟᴏɴᴇ</b>\n💾 Size</b>: {download.size()}"
                    msg += f"\n<b>⚡ Sᴘᴇᴇᴅ :</b> {download.speed()}" \
                            f"\n<b>⏲️ Eᴛᴀ :</b> {download.eta()} "
                    # if hasattr(download, 'is_torrent'):
                    try:
                        msg += f"\n<b>👥 Uꜱᴇʀ :</b> <b>{download.message.from_user.first_name}</b>\n<b>⚠️ Warn:</b><code>/warn {download.message.from_user.id}</code>"
                    except:
                        pass
                    try:
                    ...
                    try:
                        msg += f"\n<b>🌱 Sᴇᴇᴅᴇʀꜱ :</b> <code>{download.aria_download().num_seeders}</code>" \
                                f"\n<b>✳️ Pᴇᴇʀꜱ :</b> <code>{download.aria_download().connections}</code>"
                    except:
                        pass
                    try:
